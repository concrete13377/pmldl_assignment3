...
def evaluate(encoder, decoder, sentence, max_length=max_len):
    ...
    with torch.no_grad():
        ...
        decoder_input = torch.tensor([[dataset.start_of_string_token_idx]], device=device)  # SOS
...
def eval_test(sequence):
    ...
    l = len(sequence)
    if l != 9:
        val_ = '<start_of_string> ' + ' '.join(sequence) + ' ' + ('<pad> ' * (10 - l - 1)) + '<end_of_string>'
    else:
...
if __name__ == '__main__':
    ...
    for epoch in range(EPOCHS):
        ...
        for i, val in progress:
            ...
            encoder_outputs = torch.zeros(max_len + 2, encoder.hidden_size, device=device)
            ...
            decoder_input = torch.tensor([[dataset.start_of_string_token_idx]], device=device)
